#!/usr/bin/env python3
"""
update_dashboard.py — Forecast Q2 2026 ARG Sales
...
PORTAL_ID   = "19835492"
PIPELINE_ID = "39845777"  # ARG Sales

# Stage mapping — ARG Sales pipeline
STAGE_NAMES = {
    "85823385":   "Discovery",
    "85823386":   "Qualified",
...
QUOTAS = {
    "Mariano Monte de Oca":  {"quota": 6000, "team": "ARG Sales", "initials": "MM", "id": "v-mariano"},
    "Matías Nisenbaum":      {"quota": 6000, "team": "ARG Sales", "initials": "MN", "id": "v-matias"},
    "Gastón Rozenblum":      {"quota": 6000, "team": "ARG Sales", "initials": "GR", "id": "v-gaston"},
    "Oriana Sarthou":        {"quota": 6000, "team": "ARG Sales", "initials": "OS", "id": "v-oriana"},
    "Dylan Liniado":         {"quota": 6000, "team": "ARG Sales", "initials": "DL", "id": "v-dylan"},
    "Santiago Mudano":       {"quota": 6000, "team": "ARG Sales", "initials": "SM", "id": "v-santi"},
    "Thiago García":         {"quota": 6000, "team": "ARG Sales", "initials": "TG", "id": "v-thiago"},
    "María Daniela Aldana":  {"quota": 6000, "team": "ARG Sales", "initials": "DA", "id": "v-dani"},
}

EXCLUDED_VENDORS = {
    "Val Sánchez", "Tobías Savich",
}

# Deals a excluir (IDs) — vacío por defecto
EXCLUDED_DEALS = set()

# Objetivos agregados ARG
Q_GOAL          = 30000   # Q2 target (10k/mes × 3 meses)
FORECAST_TARGET = 60000   # 2× objetivo Q

ARG_NAMES = list(QUOTAS.keys())
...
# Canonical name corrections — maps exact HubSpot name → display name in QUOTAS
NAME_CORRECTIONS = {
    "María Daniela De Los Angeles Aldana Ruiz": "María Daniela Aldana",
    "Maria Daniela De Los Angeles Aldana Ruiz": "María Daniela Aldana",
    "Matias Nisenbaum":      "Matías Nisenbaum",
    "Gaston Rozenblum":      "Gastón Rozenblum",
    "Thiago Garcia":         "Thiago García",
...
def stage_badge_html(stage_id, stage_name):
    # Unificar las 3 etapas como "Verbal Win"
    if stage_id in VERBAL_WIN:
        return '<span class="badge badge-vw">Verbal Win</span>'
    STAGE_COLORS = {
        "85823385":   ("#0052cc20", "#0052cc", "#0052cc40"),  # Discovery
        "85823386":   ("#36b37e20", "#36b37e", "#36b37e40"),  # Qualified
        "114785579":  ("#00b8d920", "#00b8d9", "#00b8d940"),  # Propuesta Comercial
        "1296021092": ("#ff991f20", "#ff991f", "#ff991f40"),  # Negotiation
        "85823390":   ("#36b37e20", "#00875a", "#36b37e40"),  # Close Won
    }
...
def process_data(owners, raw_q2, raw_new):
    ...
    arg = {k: team_agg(ARG_NAMES, k) for k in ["vw", "pipeline", "cw"]}
    ...
    for d in raw_new:
        ...
        display_stage = "Verbal Win" if sid in VERBAL_WIN else STAGE_NAMES.get(sid, sid or "—")
        ng_deals.append({
...
def build_html(data, update_time):
    ...
    # ── TAB 1: Team (ARG Sales) ──────────────────────────────────────────────
    arg_fc = arg["vw"] + arg["pipeline"] + arg["cw"]
    arg_2x = Q_GOAL * 2
    ...
    proj_vs_goal_pct = pct(proj_cw_q2, Q_GOAL)
    proj_cls = "delta-up" if proj_cw_q2 >= Q_GOAL else ("delta-flat" if proj_cw_q2 >= Q_GOAL*0.8 else "delta-down")
    proj_arrow = "▲" if proj_cw_q2 >= Q_GOAL else "▼"
    ...
    for rank_i, (ln, latt, lcw, lq) in enumerate(top3):
        _tclr = "#0052cc"  # ARG Sales usa azul
...
<div class="vendor-section" id="{vid}" style="display:{section_display}">
  <div class="vendor-header">
    <div class="vh-left">
      ...
      <div>
        ...
        <div class="vh-meta"><span class="vh-team-badge" style="background:{_team_bg};color:{_team_fg}">ARG Sales</span> · Cuota Q2: <strong>{usd(quota)}</strong></div>
      </div>
    </div>
    <div class="vh-right">
  ...
  <div class="kpi-grid" style="grid-template-columns:repeat(3,1fr);margin-bottom:14px">
    ...
    <div class="kpi k-purple">
      ...
      <div class="kpi-sub">Verbal Win + Internal Validation + Contrato firma</div>
    </div>
...
<title>Forecast Q2 2026 — ARG Sales</title>
...
<header class="header">
  <div>
    <div class="header-eyebrow"><span class="dot"></span>Mendel · ARG Sales · Live</div>
  ...
  <div class="header-right">
    ...
    <div class="hr-value">{usd(Q_GOAL)} <span class="hr-unit">USD</span></div>
    <div class="hr-sub">Close Won actual: {usd(cw_t)} · {pct(cw_t, Q_GOAL)}%</div>
  </div>
</header>
...
<main class="page active" id="overview">
  ...
  <div class="kpi-grid">
    <div class="kpi k-blue">
      ...
      <div class="kpi-sub">Objetivo: {usd(Q_GOAL)} · {pct(cw_t, Q_GOAL)}% completado</div>
    </div>
    <div class="kpi k-purple">
      <div class="kpi-label">⭐ Verbal Win</div>
      <div class="kpi-value" style="color:var(--purple)">{usd(vw_t)}</div>
      <div class="kpi-sub">Verbal Win + Internal Validation + Contrato firma · {len(d["vw_deals"])} negocios</div>
    </div>
    <div class="kpi k-teal">
      <div class="kpi-label">📦 Pipeline Activo</div>
      <div class="kpi-value">{usd(pipe_t)}</div>
      <div class="kpi-sub">Discovery · Qualified · Propuesta · Negotiation · {len(d["pipe_deals"])} negocios</div>
    </div>
  ...
  <div class="progress-card">
    <div>
      ...
      <div class="prog-val" style="color:{'var(--green-dk)' if cw_t >= Q_GOAL else 'var(--red-dk)'}">{pct(cw_t,Q_GOAL)}% — {usd(cw_t)} / {usd(Q_GOAL)}</div>
      <div class="prog-bar"><div class="prog-fill" style="width:{bw(cw_t,Q_GOAL)}%;background:var(--green)"></div></div>
      <div class="prog-meta"><span>$0</span><span>{usd(Q_GOAL//2)}</span><span><strong>{usd(Q_GOAL)} objetivo</strong></span></div>
    </div>
    <div>
  ...
  <!-- Equipo único ARG Sales -->
  <div class="team-card" style="margin-bottom:18px">
    <div class="team-name" style="color:#0052cc">🇦🇷 ARG Sales <span style="font-size:.75rem;font-weight:500;color:var(--muted);margin-left:6px">Obj Q: {usd(Q_GOAL)} · Obj mensual: $10,000 · Obj vendedor: $6,000</span></div>
    <div class="team-row">
      <div class="team-row-header">
        <span class="team-row-label">🏆 Close Won</span>
        <span class="team-row-val" style="color:{'var(--green-dk)' if arg['cw']>=Q_GOAL else 'var(--red-dk)'}">{usd(arg['cw'])} / {usd(Q_GOAL)}</span>
      </div>
      <div class="prog-bar"><div class="prog-fill" style="width:{bw(arg['cw'],Q_GOAL)}%;background:var(--green)"></div></div>
      <div class="prog-meta"><span>{pct(arg['cw'],Q_GOAL)}% completado</span><span>Q2 Abril–Junio</span></div>
    </div>
    <div class="team-row">
      <div class="team-row-header">
        <span class="team-row-label">📊 Forecast vs 2× objetivo ({usd(arg_2x)})</span>
        <span class="team-row-val" style="color:{'var(--green-dk)' if arg_fc >= arg_2x else 'var(--red-dk)'}">{pct(arg_fc,arg_2x)}%</span>
      </div>
      <div class="prog-stacked">
        <div style="width:{bw(arg['cw'],arg_2x)}%;background:var(--green);min-width:{2 if arg['cw'] else 0}px"></div>
        <div style="width:{bw(arg['vw'],arg_2x)}%;background:var(--purple);min-width:{2 if arg['vw'] else 0}px"></div>
        <div style="width:{bw(arg['pipeline'],arg_2x)}%;background:var(--teal)"></div>
      </div>
      <div class="prog-meta">
        <span><span style="color:var(--purple)">■</span> VW {usd(arg['vw'])} &nbsp;<span style="color:var(--teal)">■</span> Pipe {usd(arg['pipeline'])}</span>
        <span>Total {usd(arg_fc)} · Brecha {usd(max(0,arg_2x-arg_fc))}</span>
      </div>
    </div>
  </div>
...
def main():
    ...
    print("Fetching Q2 deals (ARG Sales)…")
    ...
    print("Fetching new deals since Dec 29 2025 (ARG Sales)…")
